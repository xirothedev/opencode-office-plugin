...
**Full plugin, no daemon**. Drop standalone daemon, wire protocol, binary release. Plugin = npm package `@xirothedev/openoffice-plugin-opencode`. Installed by adding it to the `plugins` array in opencode 2 config (see docs/INSTALL.md). Targets the opencode V2 plugin API (ADR-0016, supersedes ADR-0010): `Plugin.define({ id: "openoffice", effect })` from `@opencode/plugin/effect`, pinned to `@opencode/plugin@2.0.10` + `@opencode/schema@2.0.10` (exact).
...
```
./
  package.json          # @xirothedev/openoffice-plugin-opencode, depends on @opencode/plugin@2.0.10 + @opencode/schema@2.0.10 + effect@4.0.0-rc.112
  src/
