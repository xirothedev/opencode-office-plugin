...
const isBlockedTool = (tool: string): boolean =>
...
describe("guard + live", () => {
  ...
  it("read live:true falls back to file when Word not running (darwin)", async () => {
    ...
    const r1 = await runTool(officecliTool, {
      action: "read",
      filePath: fallbackFile,
    });
    const r2 = await runTool(officecliTool, {
      action: "read",
      filePath: fallbackFile,
      live: true,
    });
  ...
  it("read live:true without draft returns file content via officecli", async () => {
    ...
    const out = await runTool(officecliTool, {
      action: "read",
      filePath: liveReadFile,
      live: true,
    });
