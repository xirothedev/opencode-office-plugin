...
mock.module("@/core/format/exec", async () => {
  ...
  return {
    runCommand: mock((cmd: string) => {
      ...
      const tmpMatch = cmd.match(/"(?<tmp>[^"]+\.tmp\.md)"/u);
      const tmp = tmpMatch?.groups?.tmp;
      if (tmp !== undefined) {
        try {
          tmpMarkdownContents.push(fs.readFileSync(tmp, "utf-8"));
        } catch {
...
describe("officecli export", () => {
  ...
  const pdfFixturePath = path.join(process.cwd(), "test/fixtures/sample.pdf");
  setupHermeticDirs();
  ...
  it("errors when targetPath is the same file as filePath", async () => {
    ...
    ).rejects.toThrow(/targetPath must differ from filePath/u);
  });
  ...
  it("errors when the source format is an image", async () => {
    ...
    await expect(
      runTool(officecliTool, {
        action: "export",
        filePath: "/tmp/test-export-image.png",
        targetPath: pdfTarget,
      })
    ).rejects.toThrow(/export source format not supported: \.png/u);
  });
  ...
  it("errors when the target format is an image", async () => {
    await expect(
      runTool(officecliTool, {
        action: "export",
        filePath: testFile,
        targetPath: "/tmp/test-export-target.png",
      })
    ).rejects.toThrow(/export target format not supported: \.png/u);
  });
  ...
  it("errors when the source file does not exist and no draft exists", async () => {
    await expect(
      runTool(officecliTool, {
        action: "export",
        filePath: "/tmp/test-export-missing.docx",
        targetPath: pdfTarget,
      })
