...
describe("substituteTemplate", () => {
  it("substitutes a single placeholder", () => {
    expect(substituteTemplate("Hello {{name}}!", { name: "Hạnh" })).toBe(
      "Hello Hạnh!"
    );
  });
  ...
  it("substitutes multiple placeholders in one pass", () => {
    expect(
      substituteTemplate("{{dept}} request #{{number}}", {
        dept: "Microbiology",
        number: "3",
      })
    ).toBe("Microbiology request #3");
  });

  it("tolerates whitespace inside the braces", () => {
    expect(substituteTemplate("Amount: {{ amount }}", { amount: "100" })).toBe(
      "Amount: 100"
    );
  });

  it("leaves text without placeholders unchanged", () => {
    expect(substituteTemplate("No variables here.", {})).toBe(
      "No variables here."
    );
  });

  it("stringifies non-string values", () => {
    expect(substituteTemplate("Total: {{total}}", { total: 42 })).toBe(
      "Total: 42"
    );
  });
