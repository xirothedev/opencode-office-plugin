...
const isFractionPoint = (value: unknown): value is { x: number; y: number } => {
...
const isFractionRect = (
  ...
  return (
    isFraction(rect.x) &&
    isFraction(rect.y) &&
    isFraction(rect.width) &&
    isFraction(rect.height)
  );
};

export const ANNOTATE_EXTENSIONS = [".png", ".jpg", ".jpeg"];

export const normalizeStampText = (text: string): string | null => {
...
export const parseAnnotationOps = (
  ext: string,
  ...
  for (let i = 0; i < parsed.length; i += 1) {
    ...
    if (entry.type === "note") {
      if (
        ...
        entry.text === "" ||
        !isFractionPoint(entry.position)
      ) {
        throw new Error(
          `note ${i} requires text and position {x, y} between 0 and 1`
        );
      }
      const op: AnnotationOp = {
        position: entry.position as AnnotationOp["position"],
        text: entry.text,
        type: "note",
      };
      ...
      if (!isFractionRect(entry.rect)) {
        throw new Error(
          `highlight ${i} requires rect {x, y, width, height} between 0 and 1`
        );
      }
      ops.push({ rect: entry.rect as AnnotationOp["rect"], type: "highlight" });
    } else {
      if (typeof entry.text !== "string" || !isFractionPoint(entry.position)) {
        throw new Error(
          `stamp ${i} requires text and position {x, y} between 0 and 1`
        );
      }
      const stampText = normalizeStampText(entry.text);
      if (!stampText) {
        throw new Error(
          `stamp ${i} text must be one of: ${STAMP_PALETTE.join(", ")}`
        );
      }
      const op: AnnotationOp = {
        position: entry.position as AnnotationOp["position"],
...
export const renderAnnotationsToImage = async (
  absolutePath: string,
  annotations: AnnotationOp[]
): Promise<void> => {
