...
const readCustomProps = async (zip: JSZip): Promise<Record<string, string>> => {
...
const readOfficeMetadata = async (zip: JSZip): Promise<FileMetadata> => {
  ...
  if (coreFile) {
    ...
    const root =
      obj["cp:coreProperties"] ??
      obj["coreProperties"] ??
      Object.values(obj).find((v) => v && typeof v === "object");
    if (root && typeof root === "object") {
...
const readPdfMetadata = async (absolutePath: string): Promise<FileMetadata> => {
...
export const parseMetadataProperties = (
...
const setCoreElement = (
  coreXml: string,
  tag: string,
  value: string
): string => {
  const escaped = escapeXml(value);
  const selfClose = new RegExp(`<${tag}[^>]*/>`, "u");
  if (selfClose.test(coreXml)) {
    return coreXml.replace(selfClose, `<${tag}>${escaped}</${tag}>`);
  }
  const pair = new RegExp(`(<${tag}[^>]*>)[\\s\\S]*?(</${tag}>)`, "u");
...
const buildCustomXml = (custom: Record<string, string>): string => {
...
const applyOfficeMetadata = async (
  absolutePath: string,
  props: FileMetadata
): Promise<void> => {
  const zip = await JSZip.loadAsync(readFileSync(absolutePath));
...
const applyPdfMetadata = async (
  ...
  if (props.author !== undefined) {
    doc.setAuthor(props.author);
  }
  if (props.subject !== undefined) {
    doc.setSubject(props.subject);
  }
...
export const applyMetadataToFile = async (
  absolutePath: string,
  props: FileMetadata
): Promise<void> => {
  const format = detectFormat(absolutePath);
