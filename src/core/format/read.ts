...
const execTrim = (cmd: string): string | undefined => {
...
const readViaAppleScript = (filePath: string): string | undefined => {
...
return content of text object of document 1 whose full name is POSIX file "${filePath.replaceAll("\\", "\\\\").replaceAll('"', '\\"')}"
on error
return missing value
end try`;
  return execTrim(`osascript -e '${script.replaceAll("'", `'"'"'`)}'`);
};

const readViaCom = (filePath: string): string | undefined => {
  ...
  return execTrim(
    `powershell -NoProfile -Command "${ps.replaceAll('"', '`"')}"`
  );
};

const tryReadLiveDocument = (filePath: string): string | undefined => {
  try {
    if (platform() === "darwin") {
      return readViaAppleScript(filePath);
    }
    if (platform() === "win32") {
      return readViaCom(filePath);
    }
  } catch {
    return undefined;
  }
...
const normalizeOcrOptions = (
  opts?: ReadOptions
):
  | { ocr?: "hosted" | "reject"; apiKey?: string; apiUrl?: string }
  | undefined => {
  if (!opts) {
    return undefined;
  }
  const { ocr: rawOcr } = opts;
  let ocr: "hosted" | "reject" | undefined;
  if (rawOcr === true) {
    ocr = "hosted";
  } else if (rawOcr === false) {
    ocr = "reject";
  } else {
    ocr = rawOcr;
  }
...
export const readRealFileAsMarkdown = async (
  filePath: string,
  opts?: ReadOptions
): Promise<string> => {
...
export const readLiveOrFileAsMarkdown = (
  filePath: string,
  live: boolean,
  opts?: ReadOptions
): Promise<string> => {
  ...
  if (liveText !== undefined) {
    return Promise.resolve(sanitizeXmlText(liveText));
  }
  return readRealFileAsMarkdown(filePath, opts);
};
