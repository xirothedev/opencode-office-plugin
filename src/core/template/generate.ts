...
const isDataObject = (value: unknown): value is TemplateData => {
...
export const parseTemplateData = (json: string): TemplateData => {
  let parsed: unknown;
  try {
...
export const assertTemplate = (templatePath: string): void => {
...
export const parseGenerateEntries = (input: {
  data?: string;
  filePath?: string;
  dataArray?: string;
  filePaths?: string;
  ...
  if (input.dataArray && input.filePaths) {
    ...
    for (let i = 0; i < dataArray.length; i += 1) {
      if (!isDataObject(dataArray[i])) {
        throw new Error(
          `dataArray entry ${i} must be a JSON object with string or number values`
        );
      }
      if (typeof filePaths[i] !== "string") {
        throw new TypeError(`filePaths entry ${i} must be a string`);
      }
